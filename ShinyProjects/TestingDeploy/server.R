...
demographics_Estimated_Population_2018 <- demographics_Indianapolis.df %>% select(c(Demographics, Population = Estimated_Population_2018))
demographics_Estimated_Population_2010 <- demographics_Indianapolis.df %>% select(c(Demographics, Population = Estimated_Population_2010)) 
demographics_Census_Population_2010 <- demographics_Indianapolis.df %>% select(c(Demographics, Population = Census_Population_2010)) 
...
server <- function(input, output, session) {
  ...
  complaintsByYear <- reactive({
    switch(
      ...
      "2018" = 2018,
      "2019" = 2019,
      "All Years" = c(2014, 2015, 2016, 2017, 2018, 2019)
  ...
  output$Complains_Allegation.geom_point <- renderPlot({
    complaints.df %>%
      ...
      ggrepel::geom_label_repel(aes(label = sprintf("%s in %s class", `Number of Occurances`, ALG_CLASS))) +
      labs(title = sprintf("Tally of allegations (sorted by class, not unique occurance) from citizen complaints against the IMPD in %s", complaintsByYear())) +
  ...
  output$UOF.df_data_table <- DT::renderDataTable(DT::datatable({
    UOF.df[, input$UOF_variables, drop = FALSE]
  }))
